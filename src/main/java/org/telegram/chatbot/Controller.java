...
public class Controller{
    ...
    public void update(Update update){
        ...
        if (!updateText.isEmpty() && !isBot){
            if (updateText.equals("/start") || updateText.equals("/start@fadf")){
            ...
            } else if (updateText.equals("/help")) {
                sendMessage(chatID, "Вот список моих комманд:\n" +
                        ...
                        "\nЛизнуть" +
                        "\nОбкончать");
            } else if (updateText.equals(commandsCapital[0]) || updateText.equals(commandsLetter[0]) && !update.message().replyToMessage().from().id().toString().isEmpty()) {
